...
const findPaymentMethodById = (req: Request, res: Response, next: NextFunction) => {
    ...
    return PaymentMethod.aggregate([
        ...
        .then((paymentMethods) => res.status(200).json({ paymentMethod: paymentMethods.at(0) }))
        .catch((error) => res.status(500).json({ error }));
};

const updatePaymentMethod = (req: Request, res: Response, next: NextFunction) => {
    const data = req.body;
    return PaymentMethod.findByIdAndUpdate(data.id, { ...data }, { returnOriginal: false })
        .then((paymentMethod) => res.status(200).json({ paymentMethod }))
        .catch((error) => res.status(500).json({ error }));
};
...
const updateStatusPaymentMethod = (req: Request, res: Response, next: NextFunction) => {
    ...
    return PaymentMethod.updateOne({ _id: id }, { status }, { returnOriginal: false })
        .then((paymentMethod) => res.status(200).json({ paymentMethod }))
        .catch((error) => res.status(500).json({ error }));
};

const getAllPaymentMethodsActive = (req: Request, res: Response, next: NextFunction) => {
    return PaymentMethod.aggregate([
        {
            $match: {
                type: 'parent',
...
export { createPaymentMethod, deletePaymentMethod, findPaymentMethodById, findAllPaymentMethod, getAllPaymentMethodsActive, updateStatusPaymentMethod, updatePaymentMethod };
