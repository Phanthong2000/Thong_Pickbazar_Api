...
const updateGroup = (req: Request, res: Response, next: NextFunction) => {
    const data = req.body;
    return Group.findByIdAndUpdate(data.id, { ...data }, { returnOriginal: false })
        .then((group) => res.status(200).json({ group }))
        .catch((error) => res.status(500).json({ error }));
};

const getGroupByName = (req: Request, res: Response, next: NextFunction) => {
    ...
    return Group.aggregate([
        ...
        .then((groups) => res.status(200).json({ group: groups.at(0) }))
        .catch((error) => res.status(500).json({ error }));
};
...
export { findAllGroup, createGroup, deleteGroup, getAllGroupsByLangCode, getGroupByIdAndLangCode, getGroupByName, updateGroup };
